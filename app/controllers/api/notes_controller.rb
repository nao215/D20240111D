module Api
  class NotesController < Api::BaseController
    ...
    before_action :doorkeeper_authorize!, only: [:create, :search, :index, :show, :update, :destroy, :autosave]
    ...
    def update
      ...
      unless note_id.to_s.match?(/\A\d+\z/)
        return render json: { error: 'Wrong format for note ID.' }, status: :unprocessable_entity
      end
      ...
      begin
        ...
        updated_note = NoteService::Update.new.update_note(
          note_id: note_id,
          title: title,
          content: content,
        ...
        error_response({ message: e.message }, :internal_server_error)
      end
    end
    ...
    def create
      ...
      if title.blank?
        return render json: { error: "The title is required." }, status: :bad_request
      elsif content.blank?
      ...
      note_service = NotesService::Create.new(user_id: user_id, title: title, content: content)
      ...
      if result[:success]
        ...
        render json: {
          status: 201,
          note: note.as_json(only: [:id, :title, :content, :user_id, :created_at, :status])
        }, status: :created
      else
        error_message = result[:message] || "An unexpected error occurred on the server."
        error_status = case error_message
                       when "User not found" then :unauthorized
                       when "Not authorized to create note" then :forbidden
                       else :unprocessable_entity
                       end
        render json: { error: error_message }, status: error_status
      end
    end
